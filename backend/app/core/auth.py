...
TOKEN_VERSION=1;MAX_TTL_SECONDS=3600;MAX_CLOCK_SKEW_SECONDS=30
class AuthenticationError(ValueError):
    def __init__(self,code="INVALID_SESSION"):super().__init__("인증 세션이 유효하지 않습니다.");self.code=code
@dataclass(frozen=True)
class Principal:
    subject:str;role:str;expires_at:int;authentication_method:str="SIGNED_SESSION"
...
def validate_secret(secret:str):
    if not secret or len(secret)<32:raise AuthenticationError("AUTH_SECRET_NOT_CONFIGURED")
def issue_session_token(subject:str,role:str,secret:str,ttl_seconds:int=900,now:int|None=None)->tuple[str,Principal]:
    ...
    ttl=max(1,min(int(ttl_seconds),MAX_TTL_SECONDS));payload={"v":TOKEN_VERSION,"sub":subject,"role":role,"iat":now,"exp":now+ttl,"jti":secrets.token_urlsafe(12)};encoded=_b64(json.dumps(payload,separators=(",",":"),sort_keys=True).encode());signature=_b64(hmac.new(secret.encode(),encoded.encode(),hashlib.sha256).digest());return encoded+"."+signature,Principal(subject,role,payload["exp"])
def verify_session_token(token:str,secret:str,now:int|None=None)->Principal:
    ...
    if data["exp"]-data["iat"]>MAX_TTL_SECONDS:raise AuthenticationError("TTL_EXCEEDS_MAXIMUM")
    if not re.fullmatch(r"[A-Za-z0-9._-]{3,64}",str(data.get("sub",""))):raise AuthenticationError("INVALID_SUBJECT")
    return Principal(data["sub"],data["role"],data["exp"])
