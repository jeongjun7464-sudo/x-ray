...
| DB | SQLAlchemy, Alembic | SQLite/PostgreSQL 모델, 감사 이벤트와 초기 migration |
| 프론트 | React 19, TypeScript, Vite, Lucide | 반응형 대시보드, 업로드, 결과, 검토 수정, 통계 |
| 보안 | 내용 기반 파일 검사, SHA-256, CORS, CSP | PHI 비저장, 요청 ID, 보안 헤더, 인메모리 rate limit, 감사 추적 |
...
7. 예측 생성과 검토 수정은 환자정보 없이 append-only 감사 이벤트로 기록한다.
