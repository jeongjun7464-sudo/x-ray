...
정합성 결과는 연구·교육용 업무지원 정보이며 의료진의 진단과 최종 의사결정을 대신하지 않는다. 규칙 기반 실패와 sLLM 제안을 구분하고, 확인할 수 없는 항목은 `NOT_VERIFIABLE`로 표시한다.
...
동일 익명 대상의 이전·현재 X-ray 분석 결과를 비교하고 촬영 방향·해상도·장비 차이를 비교 제한으로 표시한다. 의료진 수정은 익명 active-learning 후보로만 저장하며 자동 재학습하지 않는다. 데이터셋 구축에서는 중복 제거, 환자 단위 분할, 다중 라벨 승인, manifest 및 버전을 추적한다. 실제 검증 정답이 없는 성능과 드리프트는 `NOT_MEASURED`로 유지하며 의료기기 문서는 검토가 필요한 초안으로 생성한다.

## 검증·재현·감사 대응 역량

의료영상 AI 파이프라인과 DICOM 비식별화부터 설명 가능한 AI, Human-in-the-loop, 모델 수명주기까지 하나의 서비스 경계로 설계했다. 요구사항과 위험을 시험 시나리오·실행 증적·결함·CAPA에 연결하고 입력/설정 provenance, 장애 격리와 제한 재시도, SHA-256 감사 패키지로 RA·QA 업무와 개발 활동의 추적성을 구현했다. 실제 의료기관 연동, 임상 검증, 외부 악성코드 검사와 관리자 IdP 재인증은 연결되지 않은 상태를 명시한다.
